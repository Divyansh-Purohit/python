...
if(sys.argv[1] not in file):
    lgExcp(f'No file with that name ({sys.argv[1]}) found in the current directory. Please correct the input and try again.')
df = pd.read_csv(inputFile)
if " " in sys.argv[2]:
    lgExcp("Weights must only be separated by ',' and not by spaces.")
weights = sys.argv[2].split(',')
if " " in sys.argv[3]:
    lgExcp("Impacts must only be separated by ',' and not by spaces.")
impacts = sys.argv[3].split(',')
for i in impacts:
    if i not in ["+","-"]:
        lgExcp("Impacts must be either '+' or '-'.")
if(len(df.columns) < 3):
    lgExcp(f"Too few columns in input file, atleast 3 columns expected, received {len(df.columns)}.")
if(len(weights) != len(impacts)):
    lgExcp("Number of weights and impacts specified should be equal.")
if(len(df.columns) - 1) != len(weights):
    lgExcp("Weights or impacts not specified for all numeric columns.")
if(type(df[df.columns[1:][0]][0]) not in [np.float64, np.float32, np.int64, np.int32]):
    lgExcp("Non numeric column types found, correct the input file.")
